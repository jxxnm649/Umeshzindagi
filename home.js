...
// ---------- Shop photos (Visit Us / Connect) ----------
async function loadShopPhotos() {

  const shopFrontImg = document.getElementById("shopFrontImg");
  const shopStreetImg = document.getElementById("shopStreetImg");

  if (!shopFrontImg && !shopStreetImg) return;
  ...
  try {
    ...
    if (shopFrontImg && data.shopFrontPhotoUrl) {
      shopFrontImg.src = data.shopFrontPhotoUrl;
    }

    if (shopStreetImg && data.shopStreetPhotoUrl) {
      shopStreetImg.src = data.shopStreetPhotoUrl;
    }
