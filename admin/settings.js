...
const shopFrontPhotoFile = document.getElementById("shopFrontPhotoFile");
const shopFrontPreview = document.getElementById("shopFrontPreview");
const shopStreetPhotoFile = document.getElementById("shopStreetPhotoFile");
const shopStreetPreview = document.getElementById("shopStreetPreview");
...
function applyToForm(settings) {
  ...
  renderPhotoPreview(shopFrontPreview, settings.shopFrontPhotoUrl);
  renderPhotoPreview(shopStreetPreview, settings.shopStreetPhotoUrl);

}

function renderPhotoPreview(container, url) {
  if (!container) return;
  container.innerHTML = url
    ? `<img src="${url}" alt="" width="120" style="border-radius:10px; display:block;">`
    : `<span style="font-size:12px; opacity:.6;">No photo uploaded yet</span>`;
}

// Live-preview a newly picked file before it's uploaded/saved.
function wireFilePreview(input, container) {
  if (!input || !container) return;
  input.addEventListener("change", () => {
    const file = input.files && input.files[0];
    if (!file) return;
    container.innerHTML = `<img src="${URL.createObjectURL(file)}" alt="" width="120" style="border-radius:10px; display:block;">`;
  });
}

wireFilePreview(shopFrontPhotoFile, shopFrontPreview);
wireFilePreview(shopStreetPhotoFile, shopStreetPreview);
...
function readFromForm() {
  return {
    ...
    shopFrontPhotoUrl: currentSettings.shopFrontPhotoUrl ?? "",
    shopStreetPhotoUrl: currentSettings.shopStreetPhotoUrl ?? ""
  };
}
...
settingsForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Upload any newly picked shop photos first; keep existing URL if none picked.
    const [newFrontUrl, newStreetUrl] = await Promise.all([
      uploadShopPhoto(shopFrontPhotoFile),
      uploadShopPhoto(shopStreetPhotoFile)
    ]);

    if (newFrontUrl) next.shopFrontPhotoUrl = newFrontUrl;
    if (newStreetUrl) next.shopStreetPhotoUrl = newStreetUrl;
    ...
    shopFrontPhotoFile.value = "";
    shopStreetPhotoFile.value = "";
    renderPhotoPreview(shopFrontPreview, currentSettings.shopFrontPhotoUrl);
    renderPhotoPreview(shopStreetPreview, currentSettings.shopStreetPhotoUrl);
